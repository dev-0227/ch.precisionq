...
function getCounts(amount,price){
  console.log(Math.floor(parseFloat(parseInt(amount)-parseInt(amount)*2.9/100-0.3)*100/price))
  return Math.floor(parseFloat(parseInt(amount)-parseInt(amount)*2.9/100-0.3)*100/price);
}
function mainFun(paramsArr){
  sendRequestWithToken('POST', localStorage.getItem('authToken'), paramsArr, "hedis/callcredit", (xhr, err) => {
    if (!err) {
      let result = JSON.parse(xhr.responseText);
      if(result['status'] == "success")
        window.location.replace("../success_payment_call_credit?t="+result['counts']);
      else{
